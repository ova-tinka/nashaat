...
BEGIN
    ...
    -- Count qualifying workouts from this week.
    -- Minimum = 5 minutes.
    -- Maximum credited duration per workout = 180 minutes.
    SELECT
        COUNT(*)::INTEGER,
        COALESCE(
            SUM(LEAST(wl.duration_minutes, 180)),
            0
        )::INTEGER
    INTO
        v_workouts_count,
    ...
    v_duration_points :=
        FLOOR(v_total_minutes / 10.0)::INTEGER;
    ...
    -- Update this user's score in every private board they belong to.
...
REVOKE ALL
ON FUNCTION public.recalculate_my_weekly_leaderboard_score()
FROM PUBLIC;
